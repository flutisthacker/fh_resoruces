function consoleprint(message, bgcolor, color) {
  console.log(`%c ${message}! `, `background: ${bgcolor}; color: ${color}`);
}

function confignet() {
  let netConfig = config.get('network');
  let addr = netConfig && netConfig.server && netConfig.server.addr
  let port = netConfig && netConfig.server && netConfig.server.port

  if (addr && port) connectToServer(addr, port);

  // let netConfig = config.get('network');
  // let addr = netConfig?.server?.addr ||"ws://localhost"
  // let port = netConfig?.server?.port || 888

  // if (addr && port) connectToServer(addr, port);
}

let working = false;
console.assert(working, "this is the reason ")
...
// console.log('%c a spicy log message ?', consoleStyles);
// var css =
